...
# upstream path: src/ida_pro_mcp/ida_mcp/api_resources.py
# adaptations vs upstream:
#   - decorators @tool / @unsafe / @ext / @resource still applied; they now
#     populate registries in our local rpc.py stub instead of upstream zeromcp.
#   - @idasync is the identity decorator (idalib runs in-process; no GUI thread).
#   - @tool_timeout records metadata but does not enforce a timeout (FastMCP
#     and the agent enforce request budgets).
#   - body of every function is unchanged from upstream so behaviour is
#     equivalent in idalib mode for non-debug tools. Debug tools (api_debug.py)
#     are best-effort under idalib (no live debugger session).
